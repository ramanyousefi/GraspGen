...
class ClutterRemovalSim(object):
    ...
    def reset(self, object_count):
        ...
        self.world.set_gravity([0.0, 0.0, 0.0])
        self.draw_workspace()
    ...
    def generate_pile_scene(self, object_count, table_height):
        ...
        for urdf in urdfs:
            ...
            rotation = Rotation.from_rotvec([90, 0, 0])

            # Instead of random, we calculate the exact middle of the workspace
            center_x = self.size / 2.0
            center_y = self.size / 2.0
            xy = np.array([center_x, center_y])

            pose = Transform(rotation, np.r_[xy, table_height + 0.03])
            scale = self.rng.uniform(0.8, 1.0)

            self.slide_body = self.world.load_urdf(urdf, pose, scale=self.global_scaling * scale)
